...
int extractTar(char tarName[])
{
	...
	if((sthe = readHeader(tarFile, &numFiles)) == NULL){
	fclose(tarFile);
   	return(EXIT_FAILURE);
	}

	for( i = 0; i < numFiles; i++ ){
		if((outFile = fopen(sthe[i].name, "w")) == NULL ){
		...
		printf("[%i]: Creando fichero %s, tamaño %i Bytes...", i, sthe[i].name, sthe[i].size);
		
		if(copynFile( tarFile, outFile, sthe[i].size ) != sthe[i].size){
